...
namespace WcfAppServer
{
    ...
    public sealed class ServiceHostAdaptor : MarshalByRefObject, IDisposable, IServiceHostAdaptor
    {
        ...
        public void Dispose()
        {
            LoggingService.Logger.Trace(string.Format("Service being disposed... {0}", ServiceId));
            Close();
        ...
        {
            LoggingService.Logger.Trace(string.Format("Service {0} created with the following endpoints:",
            ...
            foreach (IWcfEndpoint endpoint in status.Endpoints)
                LoggingService.Logger.Trace(endpoint.ToString());
        }

        public static void OnAction(object o, ServiceActionEventArgs e)
        {
            LoggingService.Logger.Info(string.Format("WCF Application Service: {0} {1}",
                                                     ((IServiceHostAdaptor) o).ServiceId, e.Action));
        }

        public static void OnActionBegin(object o, ServiceActionEventArgs e)
        {
            LoggingService.Logger.Trace(string.Format("WCF Application Service: {0} {1}",
                                                      ((IServiceHostAdaptor) o).ServiceId, e.Action));
        }
    }
}