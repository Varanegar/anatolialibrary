...
namespace Thunderstruck.Provider
{
    public abstract class DefaultProvider 
    {
        ...
        public TOut Execute<TOut>(string command, object[] commandParams)
            where TOut : class, new()
        {
            IDbCommand dbCommand = CreateDbCommand(command, commandParams);
            var execResult = dbCommand.ExecuteNonQuery();

            return ExtractOutputParameterValues<TOut>(dbCommand);
        }

        private TOut ExtractOutputParameterValues<TOut>(IDbCommand dbCommand)
            where TOut : class, new()
        {
            TOut result = new TOut();
            foreach (IDataParameter parameter in dbCommand.Parameters)
                if (parameter.Direction == ParameterDirection.Output)
                    result.GetType().GetProperty(parameter.ParameterName.Substring(1)).SetValue(result, (parameter.Value.GetType() == typeof(System.DBNull)) ? null : parameter.Value);
            return result;
        }
