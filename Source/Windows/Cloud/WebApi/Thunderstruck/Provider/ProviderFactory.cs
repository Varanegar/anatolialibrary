...
namespace Thunderstruck.Provider
{
    ...
    {
        public static Func<string, SqlProvider> CustomProvider { get; set; }
        ...
        public SqlProvider ResolveDataProvider(string providerName)
        {
            if (CustomProvider != null) return CustomProvider(providerName);

            if (Providers.ContainsKey(providerName))
            {
                return (SqlProvider)Activator.CreateInstance(Providers[providerName]);
            }
