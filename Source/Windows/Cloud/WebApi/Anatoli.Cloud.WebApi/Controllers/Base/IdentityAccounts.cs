...
namespace Anatoli.Cloud.WebApi.Controllers
{
    ...
    public class IdentityAccountsController : AnatoliApiController
    {
        ...
        private async Task<IHttpActionResult> RegisterUserOnCloudnIdentityServer(CreateUserBindingModel model, bool isCustomer = true)
        {
            using (var transaction = CurrentContext.Database.BeginTransaction(IsolationLevel.ReadCommitted))
            {
                try
                {
                    ...
                    if (isCustomer)
                        await AddCustomerInfo(model, user, CurrentContext);

                    await SendSmsNotification(model, CurrentContext, user);
        ...
        private async Task AddCustomerInfo(CreateUserBindingModel model, User user, AnatoliDbContext context)
        {
            var customer = new CustomerViewModel
            {
                ...
                Email = model.Email,
                CompanyId = DataOwnerKey
            };

            await new CustomerDomain(OwnerKey, DataOwnerKey, DataOwnerCenterKey, context)
                     .PublishAsync(new CustomerProxy().ReverseConvert(new List<CustomerViewModel> { customer }));

            var basketDomain = new BasketDomain(OwnerKey, DataOwnerKey, DataOwnerCenterKey, context);

            await basketDomain.PublishAsync(new BasketProxy().ReverseConvert(new List<BasketViewModel> {
                new BasketViewModel(BasketViewModel.CheckOutBasketTypeId, customer.UniqueId),
                new BasketViewModel(BasketViewModel.FavoriteBasketTypeId, customer.UniqueId)
            }));
        }
